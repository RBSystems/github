...
target_atom_id = 5
mass_target = 1.008
mass_total = 91.224*4 + mass_target*7
vector_target = 0.21 # amplitude of target polar vector
...
eng_frac= mass_target/mass_total
...
#poly_params = eng_frac*ev2hartree*np.array([-6.0/(angstrom2bohr*eigvec_norm[target_atom_id])**4, 2326.0/(angstrom2bohr*eigvec_norm[target_atom_id])**2])
#poly_params = [-0.5*6.0*ev2hartree/angstrom2bohr**4, 0.5*2326*ev2hartree/angstrom2bohr**2]
...
#poly_params = eng_frac*np.array([-1.72351378e-05,  2.39343109e-02])
poly_params = [-3.98858475e-05,  1.48357982e-03]
