...
titles={
    ...
    8:'Particle in a customized well'}
##################################
#FUNCTIONS
##################################
########
...
def output(Case,input_fields,input_values,E,n):
    ...
    count=0
    for i in range(n-1):
        print_center_text('E({})='.format(i) + str(E[i])+'   Delta_E({})='.format(i) + str(E[i+1]-E[i]))
    print '*'*79
    print ""
########
# SHARED FUNCTIONS
...
def harmonic_potential(x,omega,D):
    ...
    pot=(D*np.cos(x/12.*np.pi-np.pi/2))**4+omega*(np.cos(x*4.))-D**4
...
# added by zjyx
def polynomial_potential(x, poly_params, D):
    if len(poly_params) == 2:
        return poly_params[0]*x**4 + poly_params[1]*x**2 - D
    elif len(poly_params) == 3:
        return poly_params[0]*x**6 + poly_params[1]*x**4 + poly_params[2]*x**2 - D
    elif len(poly_params) == 4:
        return poly_params[0]*x**8 + poly_params[1]*x**6 + poly_params[2]*x**4 + poly_params[3]*x**2 - D
    else:
        print("Error: wrong polynomial parameters.\n")
        sys.exit(1)
...
def morse_function(a,D,x):
	x_0=9.
	return D*(np.exp(-2*a*x)-2*np.exp(-a*x))+D*0.99*(np.exp(2*a*(x-x_0))-2*np.exp(a*(x-x_0)))
...
# PLOTTING
########
def infinite_well_plot_cust(E,V,xvec,W,steps,n,Case,Uask_to_save=False,ask_squared=False):
    if ask_squared:
