...
#    pp_all_filenames = [pp_map[x] for x in species]
    ...
    # find atoms not in periodical table, marked with negative number
    numbers = []
    is_unusual = False
    for x in species:
        if x in symbol_map:
...
#show the structure of the given material
def get_rmg_structure(cell, tags):
    ...
    lines += 'a_length="%12.7f"\n\n'%(lattice[0][0])
    lines += 'b_length="%12.7f"\n\n'%(lattice[1][1])
    lines += 'c_length="%12.7f"\n\n'%(lattice[2][2])
    lines += 'crds_units="%s"\n\n'%(units)
...
class RmgIn:
    ...
    def __init__(self, lines):
        ...
        #values is just the tmp iteration of tags
        self._values = None
        self._collect(lines)
        #print self._tags
        #print self._values
    ...
    def _collect(self, lines):
        ...
        # these values need to be found first, they are used in other functions
    ...
    def _set_positions(self):
        ...
        positions = []
        movable = []
        natom = len(pos_vals)/5
        if natom%1:
            print ('Error in getting positions!\n')
            sys.exit(1)
        if crd_type == 'Cell Relative':
            for i in range(natom):
                positions.append([pos_vals[i*5],
                                 [float(x) for x in pos_vals[i*5+1:i*5+4]]])
                                 #int(pos_vals[i*5+4])])
                movable.append(int(pos_vals[i*5+4]))
        elif crd_type == 'Absolute':
            self._tags['atomic_coordinate_type'] = 'Cell Relative'
            for i in range(natom):
                positions.append([pos_vals[i*5],
                                 [float(pos_vals[i*5+1])/self._tags['a_length'],
                                  float(pos_vals[i*5+2])/self._tags['b_length'],
                                  float(pos_vals[i*5+3])/self._tags['c_length']]])
                movable.append(int(pos_vals[i*5+4]))
        else:
