...
def check_stats():
    ...
    log_file = 'supercell.in.%02d.log'% 0
    if not os.path.exists(log_file):
        msg = 'supercell.in.00.log not found.'
        return False, msg
    elif os.path.exists('supercell.in.%02d.log'% 1):
        msg = 'multiple log files found.'
        return False, msg

    #content = os.popen('grep "TIMING" %s'% log_file).read()
    content = os.popen('grep "successfully" %s'% log_file).read()
    if content == "":
