        ...
        if 'IONIC POSITIONS' in log_lines[i]:
            for j in range(i+1, len(log_lines)):
                if 'Species' in log_lines[j]:
                    pos_flag = j
        #if 'Basis' in log_lines[i]:
        #    if 'X' in log_lines[i]:
        #        a_length = float(log_lines[i].split()[3])
        #    elif 'Y' in log_lines[i]:
        #        b_length = float(log_lines[i].split()[4])
        #    elif 'Z' in log_lines[i]:
        #        c_length = float(log_lines[i].split()[5])
