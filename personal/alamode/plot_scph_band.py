...
if __name__ == '__main__':
    ...
    data_merged = np.loadtxt(files[0], dtype=float, usecols=tuple(range(1,17))).reshape((11, 505, 16))
    #print data_merged[0]
    #for file in files:
    ...
    for i in range(len(data_merged)):
        color = colormap(normalize(100+100*i))
        for j in range(1, len(data_merged[i][0][0:])):
