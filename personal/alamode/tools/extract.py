...
import interface.VASP2 as vasp2
import interface.RMG as rmg
import interface.QE as qe
import interface.CASTEP as castep
...
parser.add_option('--VASP2',
                  metavar='orig.POSCAR',
                  help="VASP POSCAR file with equilibrium atomic \
                        positions (default: None), reading OUTCAR.")

parser.add_option('--RMG',
                  metavar='orig.rmg.in',
                  help="RMG input file with equilibrium atomic \
                        positions (default: None)")

parser.add_option('--CASTEP',
                  metavar='orig.castep',
                  help="CASTEP input file with equilibrium atomic \
                        positions (default: None)")
...
$ python displace.py -h")
    ...
    conditions = [options.VASP is None,
                  options.VASP2 is None,
                  options.RMG is None,
                  options.CASTEP is None,
                  options.QE is None,
    ...
    if conditions.count(True) == len(conditions):
        print("Error : Either --VASP, --RMG, --CASTEP, --QE, --xTAPP, --LAMMPS, --OpenMX option must be given.")
        exit(1)

    elif len(conditions) - conditions.count(True) > 1:
        print("Error : --VASP, --RMG, --CASTEP, --QE, --xTAPP, --LAMMPS, and --OpenMX cannot be given simultaneously.")
        exit(1)
    ...
    if code == "VASP":
        ...
        aa, aa_inv, elems, nats, x_frac0 = vasp2.read_POSCAR(file_original)

    elif code == "RMG":
        aa, elems, nats, x_frac0 = rmg.read_rmg_input(file_original)

    elif code == "CASTEP":
        aa, elems, nats, x_frac0 = castep.read_castep_input(file_original)
        #aa, elems, nats, x_frac0 = pdb.read_input(file_original)
    ...
    if print_disp:
        if code == "VASP":
            ...
            vasp.print_displacements_VASP2(file_results, aa, np.sum(nats), x_frac0,
                                     convert_unit, disp_conv_factor, options.offset)

        elif code == "RMG":
            rmg.print_displacements_RMG(file_results, aa, np.sum(nats), x_frac0,
                                     convert_unit, disp_conv_factor, options.offset)

        elif code == "CASTEP":
            castep.print_displacements_CASTEP(file_results, aa, np.sum(nats), x_frac0,
                                     convert_unit, disp_conv_factor, options.offset)
        ...
        if code == "VASP":
            ...
            vasp.print_atomicforces_VASP2(file_results, np.sum(nats),
                                    convert_unit, force_conv_factor, options.offset)

        elif code == "RMG":
            rmg.print_atomicforces_RMG(file_results, np.sum(nats),
                                    convert_unit, force_conv_factor, options.offset)

        elif code == "CASTEP":
            castep.print_atomicforces_CASTEP(file_results, np.sum(nats),
                                    convert_unit, force_conv_factor, options.offset)
        ...
        if code == "VASP":
            ...
            vasp.print_energies_VASP2(file_results, convert_unit,
                                energy_conv_factor, options.offset)

        elif code == "RMG":
            rmg.print_energies_RMG(file_results, convert_unit,
                                energy_conv_factor, options.offset)

        elif code == "CASTEP":
            rmg.print_energies_CASTEP(file_results, convert_unit,
                                energy_conv_factor, options.offset)
